<template>
<div>
  <el-dialog title="修改汉文化活动" width="40%" :destroy-on-close="true" :visible.sync="updateVisible" :before-close="handleClose">
...
<script>
  ...
  import {getSysCultureById,editSysCulture} from '../../../api/api'
  export default {
    mixins: [mixin],
    data() {
      return{
    ...
    methods: {
      query() {
        if(this.updateId) {
          getSysCultureById({id:this.updateId}).then(res => {
            if(res.code == 1000) {
              this.form = res.data
              // 处理活动时间回显
              if(res.data.activityTime && res.data.activityTime.includes(' 至 ')) {
                var timeRange = res.data.activityTime.split(' 至 ')
                this.form.activityTimeRange = timeRange
              }
              if(res.data.images) {
      ...
      submit() {
        ...
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            editSysCulture(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify({
                  title: '成功',
                  message: '修改成功',
