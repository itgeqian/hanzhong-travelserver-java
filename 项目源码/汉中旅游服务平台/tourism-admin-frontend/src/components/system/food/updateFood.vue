<template>
<div>
  <el-dialog title="修改美食推荐" width="40%" :destroy-on-close="true" :visible.sync="updateVisible" :before-close="handleClose">
...
<script>
  ...
  import {getSysFoodById,editSysFood} from '../../../api/api'
  export default {
    mixins: [mixin],
    data() {
      return{
    ...
    methods: {
      query() {
        if(this.updateId) {
          getSysFoodById({id:this.updateId}).then(res => {
            if(res.code == 1000) {
              this.form = res.data
              // 处理营业时间回显
              if(res.data.businessHours && res.data.businessHours.includes('-')) {
                var timeRange = res.data.businessHours.split('-')
                this.form.businessHoursRange = timeRange
              }
              if(res.data.images) {
      ...
      submit() {
        ...
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            editSysFood(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify({
                  title: '成功',
                  message: '修改成功',
