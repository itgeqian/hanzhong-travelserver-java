<template>
<div>
  <el-dialog title="新增联系我们" width="40%" :destroy-on-close="true" :visible.sync="addVisible" :before-close="handleClose">
...
<script>
  ...
  import {saveSysContact} from '../../../api/api'
  export default {
    mixins: [mixin],
    props: ['addVisible'],
    data() {
    ...
    methods: {
      ...
      submit() {
        ...
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            saveSysContact(this.form).then(res => {
              if(res.code == 1000) {
                this.$notify({
                  title: '成功',
                  message: '新增成功',
