...
<script>
  import add from '../../../components/system/culture/addCulture'
  import update from '../../../components/system/culture/updateCulture'
  import {getSysCulturePage,removeSysCulture,editSysCulture} from '../../../api/api' 
  export default {
    data() {
      return{
        loading: true,
    ...
    methods: {
      ...
      deleteDate(id) {
        removeSysCulture({ids:id}).then(res => {
          if(res.code == 1000) {
            this.$notify({
              title: '成功',
              message: '删除成功',
      ...
      deleteDataBtn() {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          removeSysCulture({ids:ids}).then(res => {
            if(res.code == 1000) {
              this.$notify({
                title: '成功',
                message: '删除成功',
